...
def index():
    db = get_db()
    products = db.execute('SELECT id, name, image_path, description, price FROM products').fetchall()
    user = g.user
...
def get_products_api():
    db = get_db()
    products = db.execute('SELECT id, name, description, price FROM products').fetchall()
...
def get_cart_items_api():
    ...
    if latest_cart_id:
        ...
        cart_items_data = [{'product_name': item['product_name'], 'product_quantity': item['product_quantity']} for item in cart_items]
...
def update_cart():
        ...
        if product_name and new_quantity:
            ...
            if cart_info:
                ...
                return jsonify({"message": "Cart not found for the specified user and product"}), 404
...
def cart():
    ...
    if g.user is not None:
        ...
        if latest_cart_id:
            ...
            return render_template('shop/cart.html', cartItems=cart_items_list, products=products)
